...
public class SpecialMonster {
    ...
    public static int specialMonsterController(Card selectedOrTargetCard, EffectPlace effectPlace, Game game) {
        if (effectPlace.equals(EffectPlace.DESTROY)) {
            ...
            if(selectedOrTargetCard.getCardName().equals("Scanner")){
                return Scanner.activeAbilityOfScanner((MonsterCard) selectedOrTargetCard);
            }
            ...
            if(MirageDragon.checkField(game.getGameBoardOfOpponentPlayerOfThisTurn())){
                return MirageDragon.abilityOfMirageDragon();
            }
        }
    }
